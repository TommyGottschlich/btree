...
namespace ds {
    ...
    template <typename T, std::size_t ORDER> bool Btree<T, ORDER>::contains(const T& value) const {
    ...
    template <typename T, std::size_t ORDER> bool Btree<T, ORDER>::remove(const T& value) {
    ...
    template <typename T, std::size_t ORDER> void Btree<T, ORDER>::print() const {
        print(root);
    }

    template <typename T, std::size_t ORDER> int Btree<T, ORDER>::height() const {
        return height(root);
    }

    template <typename T, std::size_t ORDER> int Btree<T, ORDER>::size() const {
        return current_size;
    }

    template <typename T, std::size_t ORDER> void Btree<T, ORDER>::clear(const bool recursive) {
        if (recursive)
    ...
    template <typename T, std::size_t ORDER> bool Btree<T, ORDER>::validate() const {
    ...
    template <typename T, std::size_t ORDER> std::string Btree<T, ORDER>::to_string() const {
        std::string result;
        if (!root)
            return result;
    ...
    template <typename T, std::size_t ORDER>
    void Btree<T, ORDER>::absorb_promoted(Node<T, ORDER>* parent, const T& promoted_key,
                                          Node<T, ORDER>* new_right_node) {
        ...
        std::copy_backward(parent->children.begin() + insert_pos + 1, parent->children.end() - 1,
                           parent->children.end());
    ...
    template <typename T, std::size_t ORDER> bool Btree<T, ORDER>::remove(Node<T, ORDER>* node, const T& value) {
        if (!node)
            return false;
    ...
    void Btree<T, ORDER>::merge(Node<T, ORDER>* parent, std::size_t child_index) {
        ...
        if (!delete_node->is_leaf()) {
            ...
            // assume merge node has orphan, (merge_node->key_count)
            std::copy(delete_node->children.begin(), delete_node->children.begin() + delete_node->key_count + 1,
                      merge_node->children.begin() + merge_node->key_count);
        }

        // step 5: copy delete_node's keys into merge_node's keys
        std::copy(delete_node->keys.begin(), delete_node->keys.begin() + delete_node->key_count,
                  merge_node->keys.begin() + merge_node->key_count);
    ...
    void Btree<T, ORDER>::borrow_from_right(Node<T, ORDER>* parent, std::size_t child_index) {
        ...
        // step 4: if right_child has children, shift children
    ...
    template <typename T, std::size_t ORDER> T Btree<T, ORDER>::inorder_successor(Node<T, ORDER>* node) const {
    ...
    template <typename T, std::size_t ORDER> int Btree<T, ORDER>::height(Node<T, ORDER>* root) const {
        if (!root)
            return 0;
    ...
    template <typename T, std::size_t ORDER> int Btree<T, ORDER>::size(Node<T, ORDER>* root) const {
        if (!root)
            return 0;
    ...
    template <typename T, std::size_t ORDER> void Btree<T, ORDER>::clear(Node<T, ORDER>* node) {
        if (!node)
            return;
    ...
    template <typename T, std::size_t ORDER> void Btree<T, ORDER>::recursive_clear(Node<T, ORDER>* node) {
        if (!node)
            return;
    ...
    template <typename T, std::size_t ORDER> void Btree<T, ORDER>::print(Node<T, ORDER>* node) const {
        if (!node)
            return;
    ...
    template <typename T, std::size_t ORDER>
    bool Btree<T, ORDER>::validate_node(Node<T, ORDER>* node, int depth, int& leaf_depth, const T* lower,
                                        const T* upper) const {
        // key count bounds
        bool is_root         = (node == root);
        std::size_t min_keys = is_root ? 1 : (ORDER / 2);

        if (node->key_count < min_keys && current_size > 1)
            return false;
        if (node->key_count > Node<T, ORDER>::MAX_KEYS)
            return false;
        ...
        for (std::size_t i = 0; i < node->key_count; ++i) {
            if (lower && node->keys[i] <= *lower)
                return false;
            if (upper && node->keys[i] >= *upper)
                return false;
            if (i + 1 < node->key_count && node->keys[i] >= node->keys[i + 1])
                return false;
        }
        ...
        for (std::size_t i = 0; i <= node->key_count; ++i) {
            ...
            const T* child_lower_bound = (i == 0) ? lower : &node->keys[i - 1];
            const T* child_upper_bound = (i == node->key_count) ? upper : &node->keys[i];

            if (!validate_node(node->children[i], depth + 1, leaf_depth, child_lower_bound, child_upper_bound))
                return false;
        }
